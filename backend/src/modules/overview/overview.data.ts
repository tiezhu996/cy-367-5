...
export function removeFromBlacklist(id: string): boolean {
  const initialLength = overviewData.blacklist.length;
  overviewData = {
    ...overviewData,
    blacklist: overviewData.blacklist.filter((item) => item.id !== id),
  };
  return overviewData.blacklist.length < initialLength;
}
